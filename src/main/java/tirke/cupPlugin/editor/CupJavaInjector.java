...
import tirke.cupPlugin.psi.impl.CupJavaBodyImpl;
...
public class CupJavaInjector implements LanguageInjector {
    ...
    public void getLanguagesToInject(@NotNull PsiLanguageInjectionHost host,
        ...
        if (host instanceof CupJavaCodeImpl) {
            // surround with class if CupJavaCodeImpl
            injectionPlacesRegistrar.addPlace(JavaLanguage.INSTANCE,
                    new TextRange(SUFFIX.length(), text.length() - SUFFIX.length()),
                    "public class Dummy {\n", "\n}");
        } else if (host instanceof CupJavaBodyImpl) {
            // surround with method if CupJavaBodyImpl
            injectionPlacesRegistrar.addPlace(JavaLanguage.INSTANCE,
                    new TextRange(SUFFIX.length(), text.length() - SUFFIX.length()),
                    "public class Dummy { public void method() {\n", "\n}}");
        }
    }
}
